...
import ProcessQueryTable from './ProcessQueryTable';
...
const rowColors = ["#fff"];
function QueryAndSolution(props) {
...
inpprops: {md:8},
  ...
  const loadInitialCustomers = useCallback(async () => {
    // const { ok } = response // BAD, DO NOT DO THIS
     const initialUnit = await post(api+"/issueSolution/loadIssueSolutionWithComments",{"issueType":"Avtivity","functionActivityId":activityId,"random":generateToken});
...
const  saveFunction = async (pallet)=>{
  ...
  const newTransac = await post(api+"/issueSolution/create"  , pallet)
...
AlertHandler("Comments Failed To Save","danger")
    ...
    function onSubmit(values) {
      console.log(values);
      values.issueType="Avtivity"
      values.functionActivityId=activityId;
      saveFunction(values)
  }
...
<details  style={{ backgroundColor: 'white' }}>
  <summary>Instructions: ALL FIELDS ARE IMPORTANT AND NEEDS TO BE FILLED IN ORDER TO PROCESS THE ORDER</summary>
    ...
    <SimpleCard md={12}>
        {<Table
          cols={ProcessQueryTable(showFormHandler, actions)}
          data={orderItems}
          rows={10}
...
export default QueryAndSolution;
