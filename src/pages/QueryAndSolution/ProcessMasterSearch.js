...
import ProcessSearchTable from './Table/ProcessSearchTable';
...
function ProcessMasterSearch(props) {
...
const { get, post, cache,response, loading, error } = useFetch({ data: [] });
  ...
  const loadInitialCustomers = useCallback(async () => {
    ...
    const initialCusts = await post(api+"/processMaster/processMaster",{"random":generateToken()});
      ...
      >
        ...
        //  watchFields={["palTrStatusId","unitName"]}
          ...
          /> </SearchCard>
            ...
            <SimpleCard md={12}>
<Table cols={ProcessSearchTable(showFormHandler, actions)}
          data={data} 
          striped
          rows={25} /> 
      </SimpleCard>
...
export default ProcessMasterSearch;
